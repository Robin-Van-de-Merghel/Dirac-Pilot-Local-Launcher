...
python $PILOT_PATH/dirac-pilot.py --modules https://github.com/DIRACGrid/DIRAC.git:::DIRAC:::dev -M 1 -S "$SETUP" -N "$CENAME" -Q "$JENKINS_QUEUE" -n "$JENKINS_SITE" --cert --certLocation=$CERT_LOCATION --wnVO="$WNVO" --pilotUUID="$pilotUUID" --debug --CVMFS_locations="$CVMFS_LOCATION/"